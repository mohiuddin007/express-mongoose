...
const createOfferedCourseIntoDB = async (payload: TOfferedCourse) => {
  ...
  const assignedSchedules = await OfferedCorse.find({
    semesterRegistration,
    faculty,
    days: { $in: days },
  ...
  const result = await OfferedCorse.create({
    ...payload,
    academicSemester,
  });
  return result;
...
const updateOfferedCourseIntoDB = async (
  ...
  const isOfferedCourseExists = await OfferedCorse.findById(id);
